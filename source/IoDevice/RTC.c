...
RTC* rtcCreate(int enable, char* cmosName)
{
    ...
    if (cmosName != NULL) {
        ...
        file = orbisOpen(cmosName, O_RDONLY,0);

        if (file > 0) {
            orbisRead(file,rtc->registers, sizeof(rtc->registers));
            orbisClose(file);
        }
...
void rtcDestroy(RTC* rtc)
{
    ...
    if (rtc->cmosName[0]) {
        int file = orbisOpen(rtc->cmosName, O_RDWR|O_CREAT,0644);

        if(file >0) {
            orbisWrite(file,rtc->registers, sizeof(rtc->registers));
            orbisClose(file);
        }
    }
