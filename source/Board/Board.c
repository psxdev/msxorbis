             ...
             int (*syncCallback)(int, int))
{
    ...
    case BOARD_MSX_FORTE_II:
        ...
        debugNetPrintf(DEBUG,"[MSXORBIS] %s msxCreate return %d\n",__FUNCTION__,success);
        break;
    case BOARD_SVI:
...
void boardChangeDiskette(int driveId, char* fileName, const char* fileInZipFile)
{
    ...
    if (boardDeviceInfo != NULL) {
        ...
        debugNetPrintf(3,"[MSXORBIS] %s boardChangeDiskette: boardDeviceInfo->disks[driveId] driveId=%d inserted=%d,name=%s,inZipName=%s\n",__FUNCTION__, driveId, boardDeviceInfo->disks[driveId].inserted,boardDeviceInfo->disks[driveId].name, boardDeviceInfo->disks[driveId].inZipName);

    }
	debugNetPrintf(3,"[MSXORBIS] %s boardChangeDiskette:%d,%s,%s\n",__FUNCTION__, driveId, fileName, fileInZipFile);
