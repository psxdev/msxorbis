...
static int readMachine(Machine* machine, const char* machineName, const char* file)
{
    ...
    for (i = 0; i < sizeof(machine->slotInfo) / sizeof(SlotInfo) && *slotBuf; i++) {
        ...
        if (!machine->isZipped)
        {
            ...
            //if (strcasecmp(machine->slotInfo[i].name, "Machines/") == 0 ||
              //  strcasecmp(machine->slotInfo[i].name, "Machines\\") == 0)
            {
                char expandedPath[1024];
                sprintf(expandedPath, "%s/%s", machinesDir, machine->slotInfo[i].name + 9);
                debugNetPrintf(DEBUG,"[MSXORBIS] %s %s\n",__FUNCTION__,expandedPath); 
...
Machine* machineCreate(const char* machineName)
{
    ...
    file = orbisOpen(configIni, O_RDONLY, 0);
    ...
    debugNetPrintf(DEBUG,"[MSXORBIS] %s readMachine return %d\n",__FUNCTION__,success);
    if (!success)
    {
		machineDestroy(machine);
        return NULL;
            ...
            if (success)
            {
                for (i = 0; i < machine->slotInfoCount; i++) {
                        ...
                        strlen(machine->slotInfo[i].inZipName))
                    {
                        ...
                        {
                            int file = orbisOpen(machine->slotInfo[i].name, O_RDONLY,0);
                            if (file <=0)
                            {
								debugNetPrintf(ERROR,"[MSXORBIS] %s File not found:%s\n",__FUNCTION__, machine->slotInfo[i].name);
                                success = 0;
                                continue;
                            }
                            orbisClose(file);
...
int machineInitialize(Machine* machine, UInt8** mainRam, UInt32* mainRamSize, UInt32* mainRamStart)
{
    ...
    for (i = 0; i < machine->slotInfoCount; i++) {
        ...
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  romName=%s \n",__FUNCTION__,romName);
    ...
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  success=%d \n",__FUNCTION__,success);
    ...
    for (i = 0; i < machine->slotInfoCount; i++) {
        ...
        debugNetPrintf(DEBUG,"[MSXORBIS] %s RAM_NORMAL success=%d \n",__FUNCTION__,success);
        ...
        debugNetPrintf(DEBUG,"[MSXORBIS] %s RAM_MAPPER success=%d \n",__FUNCTION__,success);
    ...
    for (i = 0; i < machine->slotInfoCount; i++) {
        ...
        debugNetPrintf(DEBUG,"[MSXORBIS] %s begore MEGA-SCSI success=%d \n",__FUNCTION__,success);
...
#endif
		case ROM_MSXBUS:
			//todo success &= romMapperMsxBusCreate(0, slot, subslot);
			break;
        ...
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  after romType=%d success=%d \n",__FUNCTION__,machine->slotInfo[i].romType,success);
    ...
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  final success=%d \n",__FUNCTION__,success);
