...
static int readFile(IniFile *iniFile)
{
    ...
    if (!iniFile->isZipped)
    {
        ...
        int f = orbisOpen(iniFile->iniFilename,O_RDONLY,0);
        if (f > 0)
        {
            length=orbisLseek(f,0,SEEK_END);
            ...
            if (length > 0) {
                ...
                {
                    debugNetPrintf(ERROR,"[MSXORBIS] %s error malloc for ini file %s\n",__FUNCTION__,iniFile->iniFilename);
                    orbisClose(f);
                    return 0;
                }
                length = orbisRead(f,iniFile->iniBuffer,length);
                if (length > 0) {
        ...
        {
            debugNetPrintf(ERROR,"[MSXORBIS] %s error opening ini file %s\n",__FUNCTION__,iniFile->iniFilename);
        }
    }
    else
    {
...
{
    int f = orbisOpen(filename, O_RDWR|O_CREAT,0777);
    if (f<=0) {
        debugNetPrintf(ERROR,"[MSXORBIS] %s error opening file %s for writing\n",__FUNCTION__,filename);
        return;
    }
    orbisWrite(f,iniFile->iniBuffer,iniFile->iniEnd - iniFile->iniBuffer);
    orbisClose(f);
}
