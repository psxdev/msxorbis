...
static int emuStartWithArguments(Properties* properties, char* commandLine, char *gamedir) {
    ...
    char    machineName[64] = "";
#ifdef ORBIS
    char    diskA[512] = "host0:app/msxorbis/diska.dsk";
    char    diskB[512] = "host0:app/msxorbis/diskb.dsk";
#else
...
int emuTryStartWithArguments(Properties* properties, char* cmdLine, char *gamedir) {
    ...
    if (cmdLine == NULL || *cmdLine == 0) {
        if (appConfigGetInt("autostart", 0) != 0) {
            debugNetPrintf(DEBUG,"[MSXORBIS] %s  autostart  \n",__FUNCTION__);

            emulatorStop();
            emulatorStart(properties->filehistory.quicksave);
        }
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  returning 0  \n",__FUNCTION__);
