...
static int timerCallback(void* timer) {
    if (properties == NULL) {
       ...
       // debugNetPrintf(INFO,"[MSXORBIS] %s properties->emulation.syncMethod %d\n",__FUNCTION__,properties->emulation.syncMethod);
      ...
      //  debugNetPrintf(INFO,"[MSXORBIS] %s framePeriod %d sysTime %d oldSysTime %d syncMethod %d\n",__FUNCTION__,framePeriod,sysTime,oldSysTime,syncMethod);
...
static void emulatorThread() {
    ...
    success = boardRun(machine,
                       ...
                       *emuStateName ? emuStateName : NULL,
                       ...
                       reverseBufferCnt,
                       WaitForSync);
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  boardRun return %d \n",__FUNCTION__,success);
...
//xxxx = 0;
    ...
    if (machine == NULL) {
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  error creating machine  \n",__FUNCTION__);
...
#ifndef NO_TIMERS
#ifndef WII
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  emuSyncEvent  \n",__FUNCTION__);
    emuSyncEvent  = archEventCreate(0);
#endif
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  emuStartEvent  \n",__FUNCTION__);
    emuStartEvent = archEventCreate(0);
#ifndef WII
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  emuTimer  \n",__FUNCTION__);
    emuTimer = archCreateTimer(emulatorGetSyncPeriod(), timerCallback);
#endif
#endif
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  emuTimer %d  \n",__FUNCTION__,emuTimer);
    ...
    if (emulationStartFailure) {
        debugNetPrintf(DEBUG,"[MSXORBIS] %s    emulationStartFailure=%d \n",__FUNCTION__,emulationStartFailure);

        archEmulationStopNotification();
        emuState = EMU_STOPPED;
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  EMU_STOPPED\n",__FUNCTION__);
    ...
    if (emuState != EMU_STOPPED) {
        ...
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  EMU_RUNNING\n",__FUNCTION__);
...
void emulatorResume() {
    ...
    if (emuState == EMU_SUSPENDED) {
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  emuState suspended %d \n",__FUNCTION__,emuState);
        ...
        archMidiEnable(1);
        emuState = EMU_RUNNING;
        debugNetPrintf(DEBUG,"[MSXORBIS] %s  emuState running %d \n",__FUNCTION__,emuState);
