...
void propInitDefaults(Properties* properties, int langType, PropKeyboardLanguage kbdLang, int syncMode, const char* themeName) 
{
    ...
    properties->emulation.speed             = 2;//50
	...
	properties->video.force4x3ratio = 0;
...
static void propLoad(Properties* properties) 
{
	...
	GET_INT_VALUE_2(propFile, video, force4x3ratio);
...
void propSave(Properties* properties) 
{
	...
	SET_INT_VALUE_2(propFile, video, force4x3ratio);
...
{
    int f;
    debugNetPrintf(DEBUG,"[MSXORBIS] %s   %s before sprintf\n",__FUNCTION__,defDir);

    sprintf(settFilename,"%s/bluemsx.ini", defDir);
    debugNetPrintf(DEBUG,"[MSXORBIS] %s  settfilename %s before orbisOpen\n",__FUNCTION__,settFilename);

    f = orbisOpen(settFilename,O_RDONLY,0);
    if (f>0) {
        orbisClose(f);
    }
    else {
    ...
    f = orbisOpen(histFilename,O_RDONLY,0);
    if (f>0) {
        orbisClose(f);
    }
    else {
