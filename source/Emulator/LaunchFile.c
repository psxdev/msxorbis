...
int cfileexists(const char * filename){
    ...
    if (file>0){
        debugNetPrintf(DEBUG,"[MSXORBIS] %s file open fd %d \n",__FUNCTION__,file);

        orbisClose(file);
        debugNetPrintf(DEBUG,"[MSXORBIS] %s return 1 \n",__FUNCTION__);

        return 1;
    }
	int dir = orbisDopen(filename);
	if (dir)
	{
		orbisDclose(dir);
		return 1;
...
int insertCartridge(Properties* properties, int drive, const char* fname, const char* inZipFile, RomType romType, int forceAutostart) {
    ...
    }
    {
        ...
        if (buf != NULL) {
			...
			debugNetPrintf(DEBUG,"[MSXORBIS] %s media guess:%s (%d), %d\n",__FUNCTION__, romName, size, chkRomType);
...
int insertDiskette(Properties* properties, int drive, const char* fname, const char* inZipFile, int forceAutostart) {
    ...
    debugNetPrintf(3,"[MSXORBIS] %s insertDiskette: fname=%s isZip=%d\n",__FUNCTION__, filename,isZip);
    ...
    if (isZip) {
        if (inZipFile != NULL) {
			...
			debugNetPrintf(3,"fileListDsk:%s, cnt=%d\n", fileListDsk, countDsk);
	...
	{
		debugNetPrintf(3,"[MSXORBIS] %s file not exists: %s\n",__FUNCTION__, filename);
		return 1;
	}
	debugNetPrintf(3,"[MSXORBIS] %s file exists: disk=%d, %s, %s\n",__FUNCTION__, drive, filename, diskName);
...
#endif
    debugNetPrintf(3,"[JANDER] a %s autostart=%d !noautostart=%d emulatorGetState=%d EMU_STOPPED=%d\n",__FUNCTION__,autostart,!noautostart,emulatorGetState(),EMU_STOPPED);

    if (autostart && !noautostart) {
        emulatorStop();
		debugNetPrintf(3,"[MSXORBIS] a %s insertDiskette::boardChangeDiskette\n",__FUNCTION__);
        boardChangeDiskette(drive, filename, isZip ? diskName : NULL);
        emulatorStart(NULL);
    }
    else if (emulatorGetState() != EMU_STOPPED) {
        emulatorSuspend();
		debugNetPrintf(3,"[MSXORBIS] b %s insertDiskette::boardChangeDiskette\n",__FUNCTION__);
        boardChangeDiskette(drive, filename, isZip ? diskName : NULL);
        emulatorResume();
    }
...
#ifdef ORBIS 
        int f = (fileName, O_RDONLY,0);
        if (f > 0) {
            if(orbisLseek(f, 0, SEEK_END)>=1024*1024){
