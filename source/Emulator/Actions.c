...
char statePrefix[64]         = "";
 ...
 void actionCartInsertFromHost(char *name,int cartNo)
 {
    //emulatorSuspend();
    boardChangeCartridge(0, ROM_UNKNOWN, name,NULL);
     emulatorResume(); 
    actionEmuResetSoft();

}
void actionDiskInsertFromHost(char *name,int drive)
{
    emulatorSuspend();
        debugNetPrintf(3,"[MSXORBIS] %s insertDiskette::boardChangeDiskette\n",__FUNCTION__);
        boardChangeDiskette(drive, name, NULL);
        emulatorResume();
            actionEmuResetSoft();
...
void actionHarddiskInsert(int diskNo)
{
    ...
    if (filename != NULL) {        
        insertDiskette(state.properties, diskNo, filename, NULL, 1);
    }
    emulatorResume();
    archUpdateMenu(0);
}
...
void actionEmuSpeedDecrease() {
    if (state.properties->emulation.speed > 0) {
        ...
        debugNetPrintf(3,"[MSXORBIS] speed %d\n",state.properties->emulation.speed);
        emulatorSetFrequency(state.properties->emulation.speed, NULL);
    }
}
